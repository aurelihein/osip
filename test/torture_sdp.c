...
main(int argc, char **argv)
{
  ...
  if (torture_file==NULL)
    {
    fprintf(stderr,"Failed to open \"torture_sdps\" file.\nUsage: %s torture_file [-v]\n", argv[0]);
    exit(1);
    }
...
test_accessor_get_api(sdp_t *sdp) {
  ...
  do {
    ...
    if (tmp!=NULL) printf("e_email:        |%s|\n",tmp);
    i++;
  } while (tmp!=NULL);
  i=0;
  do {
    tmp = sdp_p_phone_get(sdp,i);
    if (tmp!=NULL) printf("p_phone:        |%s|\n",tmp);
    i++;
  } while (tmp!=NULL);
  ...
  if (tmp!=NULL) printf("c_connection:   |%s| |%s| |%s| |%s| |%s|\n",
  ...
  do {
    ...
    if (tmp!=NULL) printf("b_bandwidth:    |%s|:|%s|\n",tmp,tmp2);
    k++;
  } while (tmp!=NULL);
  ...
  do {
    ...
    if (tmp!=NULL) printf("t_descr_time:   |%s| |%s|\n",tmp,tmp2);
    i=0;
    do {
      tmp2 = sdp_r_repeat_get(sdp, k, i);
      i++;
      if (tmp2!=NULL) printf("r_repeat:    |%s|\n",tmp2);
    } while (tmp2!=NULL);
    k++;
  } while (tmp!=NULL);
  ...
  while (!sdp_endof_media(sdp,i)) {
    ...
    if (tmp!=NULL) printf("m_media:        |%s| |%s| |%s| |%s|",
			  tmp,tmp2,tmp3,tmp4);
    k=0;
    do {
      tmp = sdp_m_payload_get(sdp,i,k);
      if (tmp!=NULL) printf(" |%s|",tmp);
      k++;
    } while (tmp!=NULL);
    printf("\n");
    k=0;
