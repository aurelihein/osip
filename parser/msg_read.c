...
#ifdef USE_TMP_BUFFER
{
  ...
  *dest = message;
  ...
  i = strcat_headers_all_on_one_line(message,sip->allows
				  ,"Allow: ",7
  ...
  i = strcat_headers_one_per_line(message,sip->call_infos
				  ,"Call-Info: ",11
  ...
  i = strcat_headers_one_per_line(message,sip->alert_infos
				  ,"Alert-Info: ",12
				 ,((int (*)(void *,char **))&call_info_2char)
				 ,&next);
  if (i!=0) { sfree(*dest);  return -1; }
  ...
  i = strcat_headers_one_per_line(message,sip->error_infos
				  ,"Error-Info: ",12
				 ,((int (*)(void *,char **))&call_info_2char)
				 ,&next);
  if (i!=0) { sfree(*dest);  return -1; }
  ...
  i = strcat_headers_all_on_one_line(message,sip->accepts
				  ,"Accept: ",8
				 ,((int (*)(void *,char **))&content_type_2char)
				 ,&next);
  if (i!=0) { sfree(*dest);  return -1; }
  ...
  i = strcat_headers_all_on_one_line(message,sip->accept_encodings
				  ,"Accept-Encoding: ",17
				 ,((int (*)(void *,char **))&accept_encoding_2char)
				 ,&next);
  if (i!=0) { sfree(*dest);  return -1; }
  message = next;

  i = strcat_headers_all_on_one_line(message,sip->accept_languages
				  ,"Accept-Language: ",17
				 ,((int (*)(void *,char **))&accept_encoding_2char)
				 ,&next);
  if (i!=0) { sfree(*dest);  return -1; }
...
msg_logresponse(sip_t *sip,char *fmt)
{
  ...
  if (1==is_trace_level_activate(TRACE_LEVEL4))
	...
	trace(__FILE__,__LINE__,TRACE_LEVEL4,stdout,"MESSAGE : Could not make a string of message!!!!\n");
    }
