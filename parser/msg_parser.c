	       ...
	       char **end_of_header)
{
  ...
  *end_of_header = NULL; /* AMD fix */
  while (1) {
    ...
    if (('\r' == soh[0]) && ('\n' == soh[1])) {  /* case 1: CRLF is the separator */
      tmp = soh+1;
      leol=2;
    } else
	...
	for(;;)
	  { /* this is for line ending with '\cr\lf\t    ' (i.e more than one space after tab) */
	    if (' ' == *extraspaces) extraspaces++; 
	    else if ('\t' == *extraspaces) { *extraspaces=' ';  extraspaces++; } 
			   ...
			   char *hvalue)
{
  ...
  if (comma==NULL||strncmp(hname,"organization",12)==0
      ||(strncmp(hname,"to",2)==0&&strlen(hname)==2)
      ||(strncmp(hname,"from",4)==0&&strlen(hname)==4)  /* AMD: BUG fix */
      ||strncmp(hname,"call-id",7)==0
      ||(strncmp(hname,"cseq",4)==0&&strlen(hname)==4)  /* AMD: BUG fix */
      ||strncmp(hname,"subject",7)==0
      ||strncmp(hname,"user-agent",10)==0
      ||strncmp(hname,"server",6)==0
      ||strncmp(hname,"www-authenticate",16)==0          /* AMD: BUG fix */
      ||strncmp(hname,"authentication-info",19)==0
      ||strncmp(hname,"proxy-authenticate",20)==0
      ||strncmp(hname,"proxy-authorization",19)==0
      ||strncmp(hname,"proxy-authentication-info",25)==0 /* AMD: BUG fix */
	       ...
	       char **next_body)
{
  ...
  if (sip->mime_version==NULL)
	...
	{
	    int body_len;
	    if (start_of_buf[0]=='\0') return -1; /* final CRLF is missing */
	  /* get rid of the first CRLF */
	  if ('\r'==start_of_buf[0]) 
	  {
	    if ('\n'==start_of_buf[1]) 	start_of_body = start_of_buf + 2;
	    else 			start_of_body = start_of_buf + 1;
	  }
	  else if ('\n'==start_of_buf[0]) start_of_body = start_of_buf + 1;
