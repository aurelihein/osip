...
quoted_string_set(char *name, char *str, char **result, char **next)
{
  *next = NULL;
  ...
  if (strnicmp(name,str,strlen(name))==0)
	...
	/* for exemple "noncecount" (found in vovida!) is deetected */
	/* as "nonce". This get ri of this behavior.                */
	...
	if (hack-str!=strlen(name))
      ...
      if (quote1==NULL) return -1;  /* bad header format... */
      quote2 = quote_find(quote1+1);
      if (quote2==NULL) return -1;  /* bad header format... */
      if (quote2-quote1==1) {
...
token_set(char *name, char *str, char **result, char **next)
{
  ...
  *next = NULL;
  ...
  beg = strchr(str,'=');
  if (beg==NULL) return -1;  /* bad header format... */

  if (strlen(str)<6) return 0; /* end of header... */
...
www_authenticate_parse(www_authenticate_t *wwwa, char *hvalue)
{
  ...
  if (space-hvalue+1<2) return -1;
  wwwa->auth_type = (char *)smalloc(space-hvalue+1);
  sstrncpy(wwwa->auth_type,hvalue,space-hvalue);

  for(;;)
    { int parse_ok=0;
      if (quoted_string_set("realm",space,&(wwwa->realm),&next)) return -1;
      if (next==NULL) return 0; /* end of header detected! */
      else if(next!=space) {
        space = next;
	parse_ok++;
      }
      if (quoted_string_set("domain",space,&(wwwa->domain),&next)) return -1;
      if (next==NULL) return 0; /* end of header detected! */
      else if(next!=space) {
        space = next;
	parse_ok++;
      }
      if (quoted_string_set("nonce",space,&(wwwa->nonce),&next)) return -1;
      if (next==NULL) return 0; /* end of header detected! */
      else if(next!=space) {
        space = next;
	parse_ok++;
      }
      if (quoted_string_set("opaque",space,&(wwwa->opaque),&next)) return -1;
      if (next==NULL) return 0; /* end of header detected! */
      else if(next!=space) {
        space = next;
	parse_ok++;
      }
      if (token_set("stale",space,&(wwwa->stale),&next)) return -1;
      if (next==NULL) return 0; /* end of header detected! */
      else if(next!=space) {
        space = next;
	parse_ok++;
      }
      if (token_set("algorithm",space,&(wwwa->algorithm),&next)) return -1;
      if (next==NULL) return 0; /* end of header detected! */ 
      else if(next!=space) {
        space = next;
	parse_ok++;
      }
      if (quoted_string_set("qop",space,&(wwwa->qop_options),&next)) return -1;
      if (next==NULL) return 0; /* end of header detected! */
      else if(next!=space) {
        space = next;
	parse_ok++;
...
void 
www_authenticate_setauth_type(www_authenticate_t *www_authenticate, char *auth_type)
{
...
void 
www_authenticate_setdomain(www_authenticate_t *www_authenticate, char *domain)
{
...
void
www_authenticate_setopaque(www_authenticate_t *www_authenticate, char *opaque)
{
...
void 
www_authenticate_setalgorithm(www_authenticate_t *www_authenticate, char *algorithm)
{
...
void 
www_authenticate_setqop_options(www_authenticate_t *www_authenticate, char *qop_options)
{
...
www_authenticate_2char(www_authenticate_t *wwwa, char **dest)
{
  ...
  char *tmp;
  *dest = NULL;
  if ((wwwa==NULL)||(wwwa->auth_type==NULL)||(wwwa->realm==NULL)||(wwwa->nonce==NULL))
    return -1;

  len = strlen(wwwa->auth_type) + 1;
  
  if (wwwa->realm!=NULL)    len = len + strlen(wwwa->realm) + 7;
  if (wwwa->nonce!=NULL)    len = len + strlen(wwwa->nonce) + 8;
  len = len + 2;
  if (wwwa->domain!=NULL)    len = len + strlen(wwwa->domain) + 9;
  if (wwwa->opaque!=NULL)    len = len + strlen(wwwa->opaque) + 9;
  if (wwwa->stale!=NULL)     len = len + strlen(wwwa->stale) + 8;
  if (wwwa->algorithm!=NULL) len = len + strlen(wwwa->algorithm) + 12;
  if (wwwa->qop_options!=NULL) len = len + strlen(wwwa->qop_options) + 6;
...
{
  if (www_authenticate==NULL) return;
...
www_authenticate_clone(www_authenticate_t *wwwa, www_authenticate_t **dest)
{
  ...
  *dest = NULL;
  if (wwwa==NULL) return -1;
  if (wwwa->auth_type==NULL) return -1;
  if (wwwa->realm==NULL) return -1;
  if (wwwa->nonce==NULL) return -1;
