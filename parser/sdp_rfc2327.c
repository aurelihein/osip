...
{
  *b = (sdp_bandwidth_t *)smalloc(sizeof(sdp_bandwidth_t));
  if (*b==NULL) return -1;
  (*b)->b_bwtype    = NULL;
  (*b)->b_bandwidth = NULL;
  return 0;
...
{
  *td = (sdp_time_descr_t *)smalloc(sizeof(sdp_time_descr_t));
  if (*td==NULL) return -1;
  (*td)->t_start_time = NULL;
  (*td)->t_stop_time = NULL;
  (*td)->r_repeats = (list_t *)smalloc(sizeof(list_t));
...
{
  if (td==NULL) return;
  sfree(td->t_start_time);
  sfree(td->t_stop_time);
...
{
  *key = (sdp_key_t *)smalloc(sizeof(sdp_key_t));
...
{
  *attribute = (sdp_attribute_t *)smalloc(sizeof(sdp_attribute_t));
...
sdp_attribute_free(sdp_attribute_t *attribute)
{
  ...
  sfree(attribute->a_att_field);
  sfree(attribute->a_att_value);
}

int
sdp_connection_init(sdp_connection_t **connection)
{
  *connection = (sdp_connection_t *)smalloc(sizeof(sdp_connection_t));
...
sdp_connection_free(sdp_connection_t *connection)
{
  ...
  sfree(connection->c_nettype);
  sfree(connection->c_addrtype);
  sfree(connection->c_addr);
  sfree(connection->c_addr_multicast_ttl);
  sfree(connection->c_addr_multicast_int);
}

int
sdp_media_init(sdp_media_t **media)
{
  *media = (sdp_media_t *)smalloc(sizeof(sdp_media_t));
  if (*media==NULL) return -1;
  (*media)->m_media = NULL;
  (*media)->m_port = NULL;
  (*media)->m_number_of_port = NULL;
  (*media)->m_proto = NULL;
  (*media)->m_payloads = (list_t *)smalloc(sizeof(list_t));
  list_init((*media)->m_payloads);
  (*media)->i_info = NULL;
  (*media)->c_connections = (list_t *)smalloc(sizeof(list_t));
  list_init((*media)->c_connections);
  (*media)->b_bandwidths = (list_t *)smalloc(sizeof(list_t));
  list_init((*media)->b_bandwidths);
  (*media)->a_attributes = (list_t *)smalloc(sizeof(list_t));
...
sdp_media_free(sdp_media_t *media)
{
  ...
  sfree(media->m_media);
  sfree(media->m_port);
  sfree(media->m_number_of_port);
  sfree(media->m_proto);
  listofchar_free(media->m_payloads);
  sfree(media->m_payloads);
  sfree(media->i_info);
  list_special_free(media->c_connections,(void *(*)(void *))&sdp_connection_free);
  sfree(media->c_connections);
  list_special_free(media->b_bandwidths,(void *(*)(void *))&sdp_bandwidth_free);
  sfree(media->b_bandwidths);
  list_special_free(media->a_attributes,(void *(*)(void *))&sdp_attribute_free);
  sfree(media->a_attributes);
...
sdp_init(sdp_t **sdp)
{
  (*sdp) = (sdp_t *)smalloc(sizeof(sdp_t));
  ...
  (*sdp)->e_emails = (list_t *) smalloc(sizeof(list_t));
  if ((*sdp)->e_emails==NULL) return -1;
  list_init((*sdp)->e_emails);

  (*sdp)->p_phones = (list_t *) smalloc(sizeof(list_t));
  if ((*sdp)->p_phones==NULL) return -1;
  list_init((*sdp)->p_phones);

  (*sdp)->c_connection = NULL;

  (*sdp)->b_bandwidths = (list_t *) smalloc(sizeof(list_t));
  if ((*sdp)->b_bandwidths==NULL) return -1;
  list_init((*sdp)->b_bandwidths);

  (*sdp)->t_descrs  = (list_t *) smalloc(sizeof(list_t));
  if ((*sdp)->t_descrs==NULL) return -1;
  list_init((*sdp)->t_descrs);

  (*sdp)->z_adjustments = NULL;
  (*sdp)->k_key = NULL;

  (*sdp)->a_attributes = (list_t *) smalloc(sizeof(list_t));
  if ((*sdp)->a_attributes==NULL) return -1;
  list_init((*sdp)->a_attributes);

  (*sdp)->m_medias = (list_t *) smalloc(sizeof(list_t));
  if ((*sdp)->m_medias==NULL) return -1;
...
sdp_parse_v(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /*v=\r ?? bad header*/
  sdp->v_version = smalloc(crlf-(equal+1)+1);
...
sdp_parse_o(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* o=\r ?? bad header*/
...
sdp_parse_s(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  sdp->s_name = smalloc(crlf-(equal+1)+1);
...
sdp_parse_u(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* u=\r ?? bad header*/

  /* u=uri */
  /* we assume this is a URI */
  sdp->u_uri = smalloc(crlf-(equal+1)+1);
...
sdp_parse_e(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* e=\r ?? bad header*/
...
sdp_parse_p(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* p=\r ?? bad header*/
...
sdp_parse_c(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* c=\r ?? bad header*/
...
sdp_parse_b(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* b=\r ?? bad header*/
...
sdp_parse_t(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* t=\r ?? bad header*/
...
sdp_parse_r(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  if (index==0) return ERR_ERROR; /* r field can't come alone! */
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* r=\r ?? bad header*/
...
sdp_parse_z(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* z=\r ?? bad header*/
...
sdp_parse_k(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* k=\r ?? bad header*/
...
sdp_parse_a(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
  ...
  while ((*crlf!='\r')&&(*crlf!='\n')&&(*crlf!='\0'))
    crlf++;
  if (*crlf=='\0') return ERR_ERROR;
  if (crlf==equal+1) return ERR_ERROR;  /* a=\r ?? bad header*/
...
sdp_parse_m(sdp_t *sdp, char *buf, char **next)
{
  ...
  *next=buf;
...

  {
    ...
    space = strchr(tmp+1,' ');
    if (space==NULL) more_space_before_crlf=1;
    else if ((space!=NULL)&&(space>crlf)) more_space_before_crlf=1;
...
sdp_parse(sdp_t *sdp, const char *buf)
{
  ...
  /* This is a simple example
    ...
    if (i==-1) /* header is bad */
      ...
      /* return -1; */
      {
	TRACE(trace(__FILE__,__LINE__,OSIP_WARNING,NULL,"The \"s\" parameter is mandatory, but this packet does not contain any! - anyway, we don't mind about it.\n"));
      }
    ptr = next_buf;
    ...
    if (list_size(sdp->e_emails)==0&&list_size(sdp->p_phones)==0)
      {
	TRACE(trace(__FILE__,__LINE__,OSIP_WARNING,NULL,"The rfc2327 says there should be at least an email or a phone header!- anyway, we don't mind about it.\n"));
      }
...
{
  if (conn->c_nettype==NULL) return -1;
  if (conn->c_addrtype==NULL) return -1;
  if (conn->c_addr==NULL) return -1;
...
{
  if (bandwidth->b_bwtype==NULL) return -1;
  if (bandwidth->b_bandwidth==NULL) return -1;
...
{
  int pos;
  if (time_descr->t_start_time==NULL) return -1;
  if (time_descr->t_stop_time==NULL) return -1;
...
{
  if (key->k_keytype==NULL) return -1;    
...
{
  if (attribute->a_att_field==NULL) return -1;
...
{
  int pos;
  if (media->m_media==NULL) return -1;
  if (media->m_port==NULL) return -1;
  if (media->m_proto==NULL) return -1;
  ...
  while (!list_eol(media->c_connections,pos))
    {
      sdp_connection_t *conn = (sdp_connection_t *)list_get(media->c_connections,pos);
      char *next_tmp2;
      int i;
  ...
  while (!list_eol(media->b_bandwidths,pos))
    {
      sdp_bandwidth_t *band = (sdp_bandwidth_t *)list_get(media->b_bandwidths,pos);
      char *next_tmp2;
      int i;
  ...
  while (!list_eol(media->a_attributes,pos))
    {
      sdp_attribute_t *attr = (sdp_attribute_t *)list_get(media->a_attributes,pos);
      char *next_tmp2;
      int i;
...
sdp_2char(sdp_t *sdp, char **dest)
{
  ...
  */
  ...
  while (!list_eol(sdp->b_bandwidths,pos))
    {
      sdp_bandwidth_t *header = (sdp_bandwidth_t *)list_get(sdp->b_bandwidths,pos);
      char *next_tmp;
      int i;
  ...
  while (!list_eol(sdp->t_descrs,pos))
    {
      sdp_time_descr_t *header = (sdp_time_descr_t *)list_get(sdp->t_descrs,pos);
      char *next_tmp;
      int i;
  ...
  while (!list_eol(sdp->a_attributes,pos))
    {
      sdp_attribute_t *header = (sdp_attribute_t *)list_get(sdp->a_attributes,pos);
      char *next_tmp;
      int i;
...
sdp_free(sdp_t *sdp)
{
  ...
  sfree(sdp->o_sess_id);
  sfree(sdp->o_sess_version);
  sfree(sdp->o_nettype);
  sfree(sdp->o_addrtype);
  sfree(sdp->o_addr);
  sfree(sdp->s_name);
  sfree(sdp->i_info);
  sfree(sdp->u_uri);
  ...
  sfree(sdp->c_connection);

  list_special_free(sdp->b_bandwidths,(void *(*)(void *))&sdp_bandwidth_free);
  sfree(sdp->b_bandwidths);

  list_special_free(sdp->t_descrs,(void *(*)(void *))&sdp_time_descr_free);
  sfree(sdp->t_descrs);

  sfree(sdp->z_adjustments);
  sdp_key_free(sdp->k_key);
  sfree(sdp->k_key);

  list_special_free(sdp->a_attributes,(void *(*)(void *))&sdp_attribute_free);
  sfree(sdp->a_attributes);
