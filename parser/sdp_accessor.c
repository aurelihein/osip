...
char *sdp_o_username_get(sdp_t *sdp) {
  if (sdp==NULL) return NULL; return sdp->o_username;
}
char *sdp_o_sess_id_get(sdp_t *sdp) {
  if (sdp==NULL) return NULL; return sdp->o_sess_id;
}
char *sdp_o_sess_version_get(sdp_t *sdp) {
  if (sdp==NULL) return NULL; return sdp->o_sess_version;
}
char *sdp_o_nettype_get(sdp_t *sdp) {
  if (sdp==NULL) return NULL; return sdp->o_nettype;
}
char *sdp_o_addrtype_get(sdp_t *sdp) {
  if (sdp==NULL) return NULL; return sdp->o_addrtype;
}
char *sdp_o_addr_get(sdp_t *sdp) {
  if (sdp==NULL) return NULL; return sdp->o_addr;
}

int   sdp_s_name_set(sdp_t *sdp, char *name) {
  if (sdp==NULL) return -1; sdp->s_name = name; return 0;
}
char *sdp_s_name_get(sdp_t *sdp) {
  if (sdp==NULL) return NULL; return sdp->s_name;
}

int   sdp_i_info_set(sdp_t *sdp, int pos_media, char *info) {
  sdp_media_t *med;
  if (sdp==NULL) return -1;
  if (pos_media==-1) { sdp->i_info = info; return 0; }
...
char *sdp_i_info_get(sdp_t *sdp,int pos_media) {
  sdp_media_t *med;
  if (sdp==NULL) return NULL; 
  if (pos_media==-1) { return sdp->i_info; }
  med = list_get(sdp->m_medias, pos_media);
...
char *sdp_u_uri_get(sdp_t *sdp) {
  if (sdp==NULL) return NULL; return sdp->u_uri;
}
...
int   sdp_c_connection_add(sdp_t *sdp, int pos_media,
  ...
  if (sdp==NULL) return -1;
  if ((pos_media!=-1)&&(list_size(sdp->m_medias)<pos_media+1)) return -1;
  ...
  if (pos_media==-1) { sdp->c_connection = conn; return 0; }
				     ...
				     char *bwtype, char *bandwidth)
{
  ...
  if (pos_media==-1) { list_add(sdp->b_bandwidths, band, -1); return 0; }
  med = (sdp_media_t*)list_get(sdp->m_medias,pos_media);
...
char *sdp_k_keytype_get(sdp_t *sdp, int pos_media) {
  ...
  if (sdp==NULL) return NULL;
  if (pos_media==-1) {
    if (sdp->k_key==NULL) return NULL;
    return sdp->k_key->k_keytype;
  }
  if ((pos_media!=-1)&&(list_size(sdp->m_medias)<pos_media+1)) return NULL;
  med = (sdp_media_t *)list_get(sdp->m_medias,pos_media);
  if (med->k_key==NULL) return NULL;
...
int   sdp_a_attribute_add(sdp_t *sdp, int pos_media, char *att_field, char *att_value){
  ...
  if (pos_media==-1) { list_add(sdp->a_attributes, attr, -1); return 0; }
  med = (sdp_media_t*)list_get(sdp->m_medias,pos_media);
