...
osip_distribute_event(osip_t *osip,sipevent_t* evt)
{
  ...
  if (EVT_IS_INCOMINGMSG(evt))
      ...
      if (transaction==NULL)
	{
	  if (EVT_IS_RCV_STATUS_1XX(evt)
	    ...
	    { /* event MUST be ignored! */
	      ...
	      TRACE(trace(__FILE__,__LINE__,OSIP_WARNING,NULL,"transaction does not yet exist... %x callid:%s\n",evt,evt->sip->call_id->number));
	      msg_free(evt->sip);
	      sfree(evt->sip);
    ...
    {
      TRACE(trace(__FILE__,__LINE__,OSIP_BUG,NULL,"wrong event type %x\n",evt));
      return NULL;
    }
}
#endif
...
osip_timers_ict_execute(osip_t *osip)
{
  ...
  while (!list_eol(osip->ict_transactions, pos))
	...
	{
	  evt = ict_need_timer_a_event(tr->ict_context, tr->state, tr->transactionid);
	  if (evt!=NULL)
	    fifo_add(tr->transactionff, evt);
	  else
	    {
...
osip_timers_nict_execute(osip_t *osip)
{
  ...
  while (!list_eol(osip->nict_transactions, pos))
	...
	{
	  evt = nict_need_timer_f_event(tr->nict_context, tr->state, tr->transactionid);
	  if (evt!=NULL)
	    fifo_add(tr->transactionff, evt);
	  else
	    {
