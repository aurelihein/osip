...
ist_init(ist_t **ist, osip_t *osip, sip_t *invite)
{
  ...
  TRACE(trace(__FILE__,__LINE__,TRACE_LEVEL3,stdout,"INFO: allocating IST context\n"));
...
{
  if (ist==NULL) return -1;
  TRACE(trace(__FILE__,__LINE__,TRACE_LEVEL3,stdout,"INFO: free ist ressource\n"));
...
ist_need_timer_g_event(ist_t *ist, state_t state, int transactionid)
{
  ...
  if (state==IST_COMPLETED)
    {
      if (ist->timer_g_start==-1) return NULL;
...
ist_need_timer_h_event(ist_t *ist, state_t state, int transactionid)
{
  ...
  if (state==IST_COMPLETED)
    {
      /* may need timer H */
      if (ist->timer_h_start==-1) return NULL;
...
ist_need_timer_i_event(ist_t *ist, state_t state, int transactionid)
{
  ...
  if (state==IST_CONFIRMED)
    {
      /* may need timer I */
      if (ist->timer_i_start==-1) return NULL;
