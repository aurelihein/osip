...
ict_init(ict_t **ict, osip_t *osip, sip_t *invite)
{
  ...
  TRACE(trace(__FILE__,__LINE__,OSIP_INFO2,NULL,"allocating ICT context\n"));
...
{
  if (ict==NULL) return -1;
  TRACE(trace(__FILE__,__LINE__,OSIP_INFO2,NULL,"free ict ressource\n"));
...
ict_need_timer_a_event(ict_t *ict, state_t state, int transactionid)
{
  ...
  if (state==ICT_CALLING)
    {
      /* may need timer A */
      if (ict->timer_a_start==-1) return NULL;
      if ((now-ict->timer_a_start)*1000>ict->timer_a_length)
	{
...
ict_need_timer_b_event(ict_t *ict, state_t state, int transactionid)
{
  ...
  if (state==ICT_CALLING)
    {
      /* may need timer B */
      if (ict->timer_b_start==-1) return NULL;
...
ict_need_timer_d_event(ict_t *ict, state_t state, int transactionid)
{
  ...
  if (state==ICT_COMPLETED)
    {
      /* may need timer D */
      if (ict->timer_d_start==-1) return NULL;
